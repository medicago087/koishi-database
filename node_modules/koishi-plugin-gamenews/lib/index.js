...
async function getNewsContent() {
  ...
  const response = await axios.get('https://www.mxnzp.com/api/news/list?typeId=515&page=1&app_id=krpxjloookipkntm&app_secret=a24rOUNiUUZGelpZTkNqL0ZPQy9FQT09');
    ...
    for (let i = 0; i < newsIds.length; i++) {
      ...
      // 判断返回数据是否有效
      if (data.data && data.data.content && data.data.content.length > 0 && !data.data.content.includes('手游') && data.data.content.length <= 3000 && !data.data.source.includes('手游') && !data.data.source.includes('NGA')) {
        // 判断是否有图片
