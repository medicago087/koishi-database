...
export class Registry<C extends Context = Context> extends Map<Plugin<C>, Runtime<C>> {
    ...
    get counter(): number;
    private resolve;
    get(plugin: Plugin<C>): Runtime<C> | undefined;
    has(plugin: Plugin<C>): boolean;
    set(plugin: Plugin<C>, state: Runtime<C>): this;
    delete(plugin: Plugin<C>): boolean;
    using(using: readonly string[], callback: Plugin.Function<void, C>): Fork<C> | undefined;
    plugin(plugin: Plugin<C>, config?: any): Fork<C> | undefined;
...
export class Fork<C extends Context = Context> extends State<C> {
    runtime: Runtime<C>;
    dispose: () => boolean;
    constructor(parent: Context, config: C['config'], runtime: Runtime<C>);
    start(): void;
...
export class Runtime<C extends Context = Context> extends State<C> {
    ...
    get name(): string;
    fork(parent: Context, config: any): Fork<C>;
    dispose(): boolean;
    setup(): void;
    private apply;
    clear(preserve?: boolean): void;
