...
// koishi/external/dialogue/packages/core/src/service.ts
...
var DialogueService = class extends import_koishi.Service {
  constructor(ctx, config) {
    super(ctx, "dialogue", true);
    this.config = config;
    this.states = /* @__PURE__ */ Object.create(null);
  ...
  async stats() {
    ...
    const [dialogues, questions] = await Promise.all([
      selection.evaluate((row) => import_koishi.$.count(row.id)).execute(),
      selection.evaluate((row) => import_koishi.$.count(row.question)).execute()
    ]);
...
__name(DialogueService, "DialogueService");
...
// koishi/external/dialogue/packages/core/src/command.ts
var import_koishi2 = require("koishi");

// koishi/external/dialogue/packages/core/src/utils.ts
...
function command(ctx, config) {
  ...
  ctx.before("parse", (content, session) => {
    ...
    if (session.parsed.prefix) {
      prefix2 = session.parsed.prefix + prefix2;
    }
...
// koishi/external/dialogue/packages/core/src/receiver.ts
...
__name(MessageBuffer, "MessageBuffer");
...
async function triggerDialogue(ctx, session, next = import_koishi3.noop) {
  ...
  if (dialogue.flag & Dialogue.Flag.regexp) {
    ...
    if (!capture)
      return;
    capture.forEach((segment4, index2) => {
      if (index2 && index2 <= 9) {
        state.answer = state.answer.replace(new RegExp(`\\$${index2}`, "g"), escapeAnswer(segment4 || ""));
      }
    });
  }
...
// koishi/external/dialogue/packages/core/src/search.ts
function apply(ctx) {
  ...
  ctx.on("dialogue/search", async (session, test, dialogues) => {
    ...
    for (const dialogue of dialogues) {
      ...
      if (parsed in options.questionMap)
        continue;
      const dialogues2 = options.questionMap[parsed] = await ctx.dialogue.get({
        ...test,
        regexp: null,
        question: parsed,
        original
...
// koishi/external/dialogue/packages/core/src/update.ts
...
function apply2(ctx) {
  ...
  ctx.on("dialogue/detail", ({ original, answer, flag }, detail, session) => {
    ...
    detail.add(session.text(".detail", [session.text(".entity.answer"), answer]), 1e3);
  });
}
__name(apply2, "apply");
...
// koishi/external/dialogue/packages/core/src/review.ts
...
// koishi/external/dialogue/packages/core/src/internal.ts
...
function apply4(ctx, config) {
  ...
  ctx.before("dialogue/action", (session) => {
    ...
    try {
      import_koishi6.segment.transform(question, {
        text: true,
        face: true,
        default: () => {
          throw new Error();
    ...
    const { original, parsed, appellative } = options.regexp ? { original: import_koishi6.segment.unescape(question), parsed: question, appellative: false } : ctx.dialogue.stripQuestion(question);
  ...
  ctx.before("dialogue/modify", async (session) => {
    ...
    function applySuggestion(session2) {
      return session2.withScope("commands.teach.messages", () => {
        return session2.argv.options.target ? analyze(session2) : create(session2);
      });
    }
...
// koishi/external/dialogue/packages/core/src/probability.ts
