...
function apply(ctx, config2 = {}) {
  const { exitCommand, autoRestart = true } = config2;
  ...
  if (exitCommand) {
    ...
    ctx.command("exit", { authority: 4 }).option("restart", "-r").action(async ({ options, session }) => {
...
(0, import_koishi2.defineProperty)(import_koishi2.Context.Config, "logger", Config);
...
Object.assign(import_koishi4.Context.Config.Advanced.dict, {
  autoRestart: import_koishi4.Schema.boolean().description("应用在运行时崩溃自动重启。").default(true).hidden(),
...
((addons2) => {
  ...
  function apply3(ctx, config2) {
    var _a;
    apply2(ctx.root);
    ctx.plugin(daemon_exports, config2);
