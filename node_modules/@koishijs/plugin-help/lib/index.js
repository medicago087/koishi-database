...
function apply(ctx, config) {
  ...
  ctx.i18n.define("en", en_US_default);
  ...
  function findCommand(target, session) {
    ...
    if (command == null ? void 0 : command.match(session))
      return command;
  ...
  const cmd = ctx.command("help [command:string]", { authority: 0, ...config }).userFields(["authority"]).userFields(createCollector("user")).channelFields(createCollector("channel")).option("authority", "-a").option("showHidden", "-H").action(async (argv, target) => {
