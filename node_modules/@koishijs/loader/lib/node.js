...
var NodeLoader = class extends import_shared.Loader {
  constructor(filename) {
    super();
    this.envData = JSON.parse(process.env.KOISHI_SHARED || "{}");
    ...
    for (const basename of ["koishi.config", "koishi"]) {
      for (const extname2 of supportedExts) {
        if (files.includes(basename + extname2)) {
          ...
          this.filename = (0, import_path.resolve)(this.baseDir, basename + extname2);
          return;
        }
      }
    }
  ...
  fullReload(code = import_shared.Loader.exitCode) {
