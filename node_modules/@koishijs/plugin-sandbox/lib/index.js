...
var SandboxBot = class extends import_koishi.Bot {
  ...
  async sendMessage(channel, content) {
    const elements = import_koishi.segment.normalize(content);
    const ids = [];
    let buffer = "";
    const flush = /* @__PURE__ */ __name(() => {
      if (!buffer.trim())
        return;
      content = import_koishi.segment.transform(buffer.trim(), {
        image(data) {
      ...
      ids.push(import_koishi.Random.id());
      buffer = "";
    }, "flush");
    const render = /* @__PURE__ */ __name((elements2) => {
...
var UserProvider = class extends import_plugin_console.DataService {
  ...
  observe(user, users) {
    ...
    users[user.sandbox] = (0, import_koishi.observe)(user, async (diff) => {
