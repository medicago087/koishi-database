...
// koishi/external/console/packages/market/src/shared/market.ts
...
// koishi/external/console/packages/market/src/shared/packages.ts
import { DataService as DataService2 } from "@koishijs/plugin-console";
var PackageProvider = class extends DataService2 {
  constructor(ctx) {
...
// koishi/external/console/packages/market/src/shared/service.ts
import { DataService as DataService3 } from "@koishijs/plugin-console";
import { Context as Context2 } from "koishi";
var ServiceProvider = class extends DataService3 {
  constructor(ctx) {
    super(ctx, "services");
  ...
  async get() {
    const services = {};
    const descriptors = Object.getOwnPropertyDescriptors(Context2.prototype);
    for (const key in descriptors) {
      const desc = descriptors[key];
      if ("value" in desc)
        continue;
      const uid = this.ctx[key]?.[Context2.source]?.state.uid;
      if (uid)
        services[key] = uid;
    }
    return services;
...
// koishi/external/console/packages/market/src/shared/writer.ts
import { DataService as DataService4 } from "@koishijs/plugin-console";
import { remove } from "koishi";
...
var ConfigWriter = class extends DataService4 {
  loader;
  plugins;
  constructor(ctx) {
...
__publicField(ConfigWriter, "using", ["console.packages"]);
export {
  ConfigWriter,
  MarketProvider,
  PackageProvider,
