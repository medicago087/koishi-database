...
var I18n = class {
  ...
  render(value, params, locale) {
    ...
    return import_core5.segment.parse(value, params).join("");
  }
...
Internal.methods = ["middleware", "component"];
    ...
    for (const shortcut of ctx.$commander._shortcuts) {
      ...
      if (typeof name === "string") {
        ...
        let escape = function(source) {
          ...
          capture.forEach((segment5, index) => {
            if (!index || index > 9)
              return;
            source = source.replace(new RegExp(`\\$${index}`, "g"), (segment5 || "").replace(/\$/g, "@@__PLACEHOLDER__@@"));
          });
...
(0, import_utils8.extend)(import_core11.Session.prototype, {
  ...
  async send(fragment, options = {}) {
    ...
    const children = await this.transform(import_core11.segment.normalize(fragment));
  ...
  async transform(elements) {
    return await import_core11.segment.transformAsync(elements, this.app.$internal._components, this);
  }
});
