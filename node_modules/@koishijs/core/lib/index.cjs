  ...
  constructor(ctx, config) {
    ...
    ctx.before("attach", (session) => {
      ...
      for (const { appel, context, i18n, fuzzy, pattern, response } of this._matchers) {
        ...
        const trigger = /* @__PURE__ */ __name((pattern2) => {
          if (!pattern2 || !response)
            return;
          let params;
          if (typeof pattern2 === "string") {
            ...
            const params2 = pattern2.exec(content);
            if (!params2)
              return;
          }
          return typeof response === "string" ? import_core6.segment.parse(response, params) : response;
        }, "trigger");
        if (!i18n) {
        ...
        for (const locale in ctx.i18n._data) {
          ...
          const result = trigger(store[pattern]);
          if (!result)
            continue;
...
Internal.methods = ["middleware", "component", "match"];
...
var _Command = class extends Argv.CommandBase {
  ...
  shortcut(pattern, config = {}) {
    ...
    for (const key in config.options || {}) {
      content += ` --${(0, import_cosmokit7.camelize)(key)}`;
...
var Commander = class {
  constructor(ctx, config = {}) {
    ...
    ctx.before("attach", (session) => {
      (0, import_cosmokit8.defineProperty)(session, "argv", ctx.bail("before-parse", session.parsed.content, session));
...
(0, import_utils7.extend)(import_core11.Session.prototype, {
  initialize() {
