...
import_core.Bot.prototype.broadcast = /* @__PURE__ */ __name(async function broadcast(channels, content, delay = this.ctx.root.config.delay.broadcast) {
  ...
  for (let index = 0; index < channels.length; index++) {
    ...
    try {
      ...
      messageIds.push(...typeof value === "string" ? await this.sendMessage(value, content) : Array.isArray(value) ? await this.sendMessage(value[0], content, value[1]) : await this.sendMessage(value.channelId, content, value.guildId, { session: value }));
    } catch (error) {
...
(0, import_cosmokit.defineProperty)(import_core2.Context.Config, "Basic", import_core2.Schema.object({
  ...
  minSimilarity: import_core2.Schema.percent().default(0.64).description("用于模糊匹配的相似系数，应该是一个 0 到 1 之间的数值。数值越高，模糊匹配越严格。设置为 1 可以完全禁用模糊匹配。")
}).description("基础设置"));
...
var FilterService = class {
  constructor(app) {
    this.app = app;
    (0, import_cosmokit3.defineProperty)(this, import_core5.Context.current, app);
    app.filter = () => true;
  ...
  get caller() {
    return this[import_core5.Context.current];
  }
  any() {
...
__name(FilterService, "FilterService");
...
import_core5.Context.service("$filter", FilterService);
...
// koishi/packages/core/src/i18n.ts
var logger = new import_core6.Logger("i18n");
...
var I18n = class {
  ...
  set(locale, prefix, value) {
    ...
    if (prefix.includes("@") || typeof value === "string") {
      ...
      if (!(0, import_cosmokit4.isNullable)(dict[path]) && !locale.startsWith("$")) {
        logger.warn("override", locale, path);
      }
      dict[path] = value;
      (_a = this[import_core6.Context.current]) == null ? void 0 : _a.on("dispose", () => {
        delete dict[path];
      });
    } else {
  ...
  render(value, params, locale) {
    ...
    return import_core6.segment.parse(value, params).join("");
  }
...
__name(I18n, "I18n");
import_core6.Context.service("i18n", I18n);
...
var Processor = class {
  constructor(ctx, config) {
    ...
    this._matchers = /* @__PURE__ */ new Set();
    (0, import_cosmokit5.defineProperty)(this, import_core7.Context.current, ctx);
    this.prepare();
  ...
  get caller() {
    return this[import_core7.Context.current];
  }
  ...
  _executeMatcher(session, matcher) {
    ...
    session.response = async () => {
      ...
      return import_core7.segment.normalize(output, params.map((source) => source ? import_core7.segment.parse(source) : ""));
    };
  }
  prepare() {
  ...
  _resolvePrefixes(session) {
    ...
    return result.map((source) => import_core7.segment.escape(source));
  }
  ...
  async _process(session, next) {
    ...
    (_a = session.elements) != null ? _a : session.elements = import_core7.segment.parse(content);
    ...
    (0, import_cosmokit5.defineProperty)(session, "parsed", { content, appel, prefix });
...
Processor.methods = ["middleware", "match"];
import_core7.Context.service("$internal", Processor);
var _keyMap;
...
(0, import_utils4.extend)(import_core8.Context.prototype, {
  get app() {
    return this.root;
  },
  get options() {
  ...
  createTimerDispose(timer) {
    const dispose = /* @__PURE__ */ __name(() => {
      ...
      if (!this.state)
        return;
      return (0, import_cosmokit6.remove)(this.state.disposables, dispose);
    }, "dispose");
...
var import_core13 = require("@minatojs/core");
...
((Argv2) => {
  ...
  class Tokenizer {
    ...
    parse(source, terminator = "") {
      const tokens = [];
      source = import_core9.segment.parse(source).map((el) => {
  ...
  createDomain("rawtext", (source) => import_core9.segment.unescape(source), { greedy: true });
  ...
  createDomain("user", (source, session) => {
    ...
    const code = import_core9.segment.from(source);
  ...
  class CommandBase {
    ...
    _createOption(name, def, config) {
      ...
      const param = (0, import_cosmokit7.paramCase)(name);
      ...
      for (let param2 of syntax.trim().split(",")) {
        ...
        if (!name2 || !param2.startsWith("-")) {
          symbols.push(import_core9.segment.escape(param2));
        } else {
          names.push(name2);
        }
      }
    ...
    parse(argv, terminator, args = [], options = {}) {
      ...
      while (!argv.error && ((_a = argv.tokens) == null ? void 0 : _a.length)) {
        ...
        if (!quoted && (option = this._symbolicOptions[content])) {
          names = [(0, import_cosmokit7.paramCase)(option.name)];
        } else {
          if (content[0] !== "-" || quoted) {
          ...
          if (content.slice(i, i + 3) === "no-" && !this._namedOptions[content.slice(i)]) {
            ...
            options[(0, import_cosmokit7.camelCase)(name)] = false;
            continue;
          }
          let j = i + 1;
        ...
        for (let j = 0; j < names.length; j++) {
          ...
          const key = optDecl ? optDecl.name : (0, import_cosmokit7.camelCase)(name);
...
// koishi/packages/core/src/command/command.ts
var logger2 = new import_core10.Logger("command");
var _Command = class extends Argv.CommandBase {
  ...
  _registerAlias(name, prepend = false) {
    ...
    if (done) {
      if (prepend) {
        (0, import_cosmokit8.remove)(this._aliases, name);
  ...
  shortcut(pattern, config = {}) {
    ...
    for (const key in config.options || {}) {
      content += ` --${(0, import_cosmokit8.camelize)(key)}`;
  ...
  before(callback, append = false) {
    ...
    (_a = this._disposables) == null ? void 0 : _a.push(() => (0, import_cosmokit8.remove)(this._checkers, callback));
    return this;
  }
  action(callback, prepend = false) {
    ...
    (_a = this._disposables) == null ? void 0 : _a.push(() => (0, import_cosmokit8.remove)(this._actions, callback));
    return this;
  }
  use(callback, ...args) {
  ...
  async execute(argv, fallback = Next.compose) {
    ...
    try {
      ...
      if (!(0, import_cosmokit8.isNullable)(result))
        return result;
    } catch (error2) {
      if (index === length)
        throw error2;
...
${stack}`);
  ...
  dispose() {
    ...
    (0, import_cosmokit8.remove)(this.ctx.$commander._commandList, this);
    if (this.parent) {
      (0, import_cosmokit8.remove)(this.parent.children, this);
    }
  }
};
...
((Command2) => {
  Command2.Config = import_core10.Schema.object({
    authority: import_core10.Schema.union([
      import_core10.Schema.natural(),
      import_core10.Schema.function()
    ]).default(1),
    hidden: import_core10.Schema.boolean().default(false),
    checkArgCount: import_core10.Schema.boolean().default(false),
    checkUnknown: import_core10.Schema.boolean().default(false)
  });
})(Command || (Command = {}));
...
var Commander = class {
  constructor(ctx, config = {}) {
    ...
    this._shortcuts = [];
    (0, import_cosmokit9.defineProperty)(this, import_core11.Context.current, ctx);
    ctx.plugin(validate);
    ...
    ctx.before("attach", (session) => {
      (0, import_cosmokit9.defineProperty)(session, "argv", ctx.bail("before-parse", session.parsed.content, session));
      session.argv.root = true;
      session.argv.session = session;
    });
    ctx.middleware((session, next) => {
      if (!session.resolveCommand(session.argv))
        return next();
  ...
  get caller() {
    return this[import_core11.Context.current];
  }
  resolve(key) {
    if (!key)
      return;
...
import_core11.Context.service(Commander.key, Commander);

// koishi/packages/core/src/session.ts
var logger3 = new import_core12.Logger("session");
var { initialize } = import_core12.Session.prototype;
(0, import_utils7.extend)(import_core12.Session.prototype, {
  [import_core12.Context.filter](ctx) {
    return ctx.filter(this);
  },
  initialize() {
    initialize.call(this);
    (0, import_cosmokit10.defineProperty)(this, "scope", null);
    (0, import_cosmokit10.defineProperty)(this, "user", null);
    (0, import_cosmokit10.defineProperty)(this, "channel", null);
    (0, import_cosmokit10.defineProperty)(this, "guild", null);
    (0, import_cosmokit10.defineProperty)(this, "_queuedTasks", []);
    (0, import_cosmokit10.defineProperty)(this, "_queuedTimeout", null);
  },
  get username() {
  ...
  async sendQueued(content, delay) {
    const text = import_core12.segment.normalize(content).join("");
    if (!text)
      return;
  ...
  resolve(source, ...params) {
    ...
    if (!(0, import_core13.isEvalExpr)(source))
      return source;
    return (0, import_core13.executeEval)({ _: this }, source);
  },
  ...
  async _observeChannelLike(channelId, fields = []) {
    ...
    if (cache) {
      ...
      cache = (0, import_utils7.observe)(data, (diff) => this.app.database.setChannel(platform, channelId, diff), `channel ${key}`);
  ...
  async observeUser(fields = []) {
    ...
    if ((_a = this.author) == null ? void 0 : _a.anonymous) {
      ...
      const user = (0, import_utils7.observe)(fallback, () => Promise.resolve());
    ...
    if (cache) {
      ...
      cache = (0, import_utils7.observe)(data, (diff) => this.app.database.setUser(this.platform, userId, diff), `user ${this.uid}`);
  ...
  text(path, params = {}) {
    ...
    const paths = (0, import_cosmokit10.makeArray)(path).map((path2) => {
      if (!path2.startsWith("."))
        return path2;
      if (!this.scope) {
  ...
  async execute(argv, next) {
    ...
    return this.withScope(`commands.${command.name}.messages`, async () => {
      ...
      if (!shouldEmit) {
        if (typeof result === "string")
          return result;
        return (0, import_core12.segment)(null, result).toString();
      }
