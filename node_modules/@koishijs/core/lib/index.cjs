...
__reExport(bot_exports, require("@satorijs/core"));
...
import_core.Bot.prototype.broadcast = /* @__PURE__ */ __name(async function broadcast(channels, content, delay = this.ctx.root.config.delay.broadcast) {
  ...
  for (let index = 0; index < channels.length; index++) {
    ...
    try {
      const [channelId, guildId] = (0, import_utils.makeArray)(channels[index]);
...
// koishi/packages/core/src/index.ts
__reExport(src_exports, bot_exports, module.exports);
...
(0, import_cosmokit.defineProperty)(import_core2.Context.Config, "Basic", import_core2.Schema.object({
  locale: import_core2.Schema.string().default("zh").description("默认使用的语言。"),
  prefix: import_core2.Schema.union([
    import_core2.Schema.array(String),
    import_core2.Schema.transform(String, (prefix) => [prefix]),
    import_core2.Schema.function()
  ]).default([""]).description("指令前缀字符构成的数组。将被用于指令的匹配。"),
  nickname: import_core2.Schema.union([
    import_core2.Schema.array(String),
    import_core2.Schema.transform(String, (nickname) => [nickname])
  ]).description("机器人昵称构成的数组。将被用于指令的匹配。"),
  autoAssign: import_core2.Schema.union([Boolean, Function]).default(true).description("当获取不到频道数据时，是否使用接受者作为代理者。"),
  autoAuthorize: import_core2.Schema.union([import_core2.Schema.natural(), Function]).default(1).description("当获取不到用户数据时默认使用的权限等级。")
}).description("基础设置"));
(0, import_cosmokit.defineProperty)(import_core2.Context.Config, "Message", import_core2.Schema.object({
  delay: import_core2.Schema.object({
    character: import_core2.Schema.natural().role("ms").default(0).description("调用 `session.sendQueued()` 时消息间发送的最小延迟，按前一条消息的字数计算。"),
    message: import_core2.Schema.natural().role("ms").default(0.1 * import_cosmokit.Time.second).description("调用 `session.sendQueued()` 时消息间发送的最小延迟，按固定值计算。"),
    cancel: import_core2.Schema.natural().role("ms").default(0).description("调用 `session.cancelQueued()` 时默认的延迟。"),
    broadcast: import_core2.Schema.natural().role("ms").default(0.5 * import_cosmokit.Time.second).description("调用 `bot.broadcast()` 时默认的延迟。"),
    prompt: import_core2.Schema.natural().role("ms").default(import_cosmokit.Time.minute).description("调用 `session.prompt()` 时默认的等待时间。")
  })
}).description("消息设置"));
(0, import_cosmokit.defineProperty)(import_core2.Context.Config, "Advanced", import_core2.Schema.object({
  prettyErrors: import_core2.Schema.boolean().default(true).description("启用报错优化模式。在此模式下 Koishi 会对程序抛出的异常进行整理，过滤掉框架内部的调用记录，输出更易读的提示信息。"),
  maxListeners: import_core2.Schema.natural().default(64).description("每种监听器的最大数量。如果超过这个数量，Koishi 会认定为发生了内存泄漏，将产生一个警告。")
}).description("高级设置"));
import_core2.Context.Config.list.push(import_core2.Context.Config.Basic, import_core2.Context.Config.Message, import_core2.Context.Config.Advanced);
...
var DatabaseService = class extends import_core3.Database {
  constructor(app) {
    super();
    this.app = app;
    (0, import_cosmokit2.defineProperty)(this, import_core4.Context.current, app);
    this.extend("user", {
      id: "string(63)",
...
DatabaseService.methods = ["getSelfIds", "broadcast"];
DatabaseService.prototype.extend = /* @__PURE__ */ __name(function extend(name, fields, config) {
  import_core3.Database.prototype.extend.call(this, name, fields, {
    ...config,
    driver: this[import_core4.Context.current].mapping.database
  });
  this.app.emit("model", name);
}, "extend");
import_core4.Context.service("database");
import_core4.Context.service("model", DatabaseService);
var defineDriver = /* @__PURE__ */ __name((constructor, schema, prepare) => ({
  ...
  apply(ctx, config) {
    config = { ...config };
    prepare == null ? void 0 : prepare(ctx, config);
...
var I18n = class {
  constructor(ctx) {
    ...
    this._presets = {};
    this.define("", { "": "" });
    this.define("zh", zh_default);
    this.define("en", en_default);
    this.define("ja", ja_default);
    this.define("fr", fr_default);
  ...
  set(locale, prefix, value) {
    ...
    if (prefix.includes("@") || typeof value === "string") {
      ...
      if (!(0, import_cosmokit3.isNullable)(dict[path]) && !locale.startsWith("$")) {
        logger.warn("override", locale, path);
      }
      dict[path] = value;
      (_a = this[import_core5.Context.current]) == null ? void 0 : _a.on("dispose", () => {
        delete dict[path];
      });
    } else {
  ...
  render(value, params, locale) {
    ...
    return value.replace(/\{(.+?)\}/g, (_, inner) => {
      ...
      for (const segment4 of segments) {
        result = result[segment4];
        if ((0, import_cosmokit3.isNullable)(result))
          return "";
      }
  ...
  registerBuiltins() {
    ...
    this.formatter("time", (ms, _, locale) => {
      ...
      return Math.round(ms / import_cosmokit3.Time.second) + " " + this.text([locale], ["general.second"], {});
    });
    ...
    this.preset("random", (data, params, locale) => {
      return this.render(import_utils2.Random.pick(data), params, locale);
    });
...
__name(I18n, "I18n");
import_core5.Context.service("i18n", I18n);
...
function createLeadingRE(patterns, prefix = "", suffix = "") {
  return patterns.length ? new RegExp(`^${prefix}(${patterns.map(import_utils3.escapeRegExp).join("|")})${suffix}`) : /$^/;
}
...
var Internal = class {
  constructor(ctx, config) {
    ...
    this.config = config;
    this._hooks = [];
    this._sessions = /* @__PURE__ */ Object.create(null);
    this._userCache = new SharedCache();
    this._channelCache = new SharedCache();
    (0, import_cosmokit4.defineProperty)(this, import_core6.Context.current, ctx);
    this.prepare();
  ...
  get caller() {
    return this[import_core6.Context.current];
  }
  middleware(middleware, prepend = false) {
    return this.caller.lifecycle.register("middleware", this._hooks, middleware, prepend);
  }
  prepare() {
    this._nameRE = createLeadingRE((0, import_utils3.makeArray)(this.config.nickname), "@?", "([,，]\\s*|\\s+)");
  }
  ...
  _stripNickname(content) {
    ...
    for (const nickname of (0, import_utils3.makeArray)(this.config.nickname)) {
  ...
  async _process(session, next) {
    ...
    (_a = session.elements) != null ? _a : session.elements = import_core6.segment.parse(content);
    let hasMention = false;
    const elements = session.elements.slice();
    while (((_b = elements[0]) == null ? void 0 : _b.type) === "at") {
      ...
      if (((_c = elements[0]) == null ? void 0 : _c.type) === "text" && !elements[0].attrs.content.trim()) {
        elements.shift();
      }
    }
    ...
    (0, import_cosmokit4.defineProperty)(session, "parsed", { content, appel, prefix });
  ...
  - ${capture ? capture[1] : lastCall.slice(7)}${midStack}`;
      ...
      try {
        ...
        let stack = (0, import_utils3.coerce)(error);
        if (prettyErrors) {
...
Internal.methods = ["middleware"];
import_core6.Context.service("$internal", Internal);
var _keyMap;
var SharedCache = class {
  constructor() {
    __privateAdd(this, _keyMap, /* @__PURE__ */ Object.create(null));
  }
  get(ref, key) {
  ...
  set(ref, key, value) {
    let entry = __privateGet(this, _keyMap)[key];
    if (entry) {
      entry.value = value;
    } else {
      entry = __privateGet(this, _keyMap)[key] = { value, key, refs: /* @__PURE__ */ new Set() };
    }
    entry.refs.add(ref);
  }
  delete(ref) {
...
(0, import_utils4.extend)(import_core7.Context.prototype, {
  get app() {
    return this.root;
  },
  get options() {
  ...
  createTimerDispose(timer) {
    const dispose = /* @__PURE__ */ __name(() => {
      ...
      if (!this.state)
        return;
      return (0, import_cosmokit5.remove)(this.state.disposables, dispose);
    }, "dispose");
...
var import_utils8 = require("@koishijs/utils");
...
var import_utils6 = require("@koishijs/utils");
...
((Argv2) => {
  ...
  let whitespace;
  ((whitespace2) => {
    whitespace2.unescape = /* @__PURE__ */ __name((source) => source.replace(/@__KOISHI_SPACE__@/g, " ").replace(/@__KOISHI_NOLINE__@/g, "\n").replace(/@__KOISHI_RETURN__@/g, "\r").replace(/@__KOISHI_TAB__@/g, "	"), "unescape");
    whitespace2.escape = /* @__PURE__ */ __name((source) => source.replace(/ /g, "@__KOISHI_SPACE__@").replace(/\n/g, "@__KOISHI_NOLINE__@").replace(/\r/g, "@__KOISHI_RETURN__@").replace(/\t/g, "@__KOISHI_TAB__@"), "escape");
  ...
  class Tokenizer {
    ...
    parseToken(source, stopReg = "$") {
      ...
      stopReg += `|${Object.keys({ ...this.bracs, ...bracs }).map(import_utils5.escapeRegExp).join("|")}`;
    ...
    parse(source, terminator = "") {
      const tokens = [];
      source = import_core8.segment.parse(source).map((el) => {
        return el.type === "text" ? el.toString() : whitespace.escape(el.toString());
      }).join("");
      let rest = source, term = "";
      const stopReg = `\\s+|[${(0, import_utils5.escapeRegExp)(terminator)}]|$`;
  ...
  createDomain("rawtext", (source) => import_core8.segment.unescape(source), { greedy: true });
  ...
  createDomain("user", (source, session) => {
    ...
    const code = import_core8.segment.from(source);
  ...
  class CommandBase {
    ...
    _createOption(name, def, config) {
      ...
      const param = (0, import_cosmokit6.paramCase)(name);
    ...
    parse(argv, terminator, args = [], options = {}) {
      ...
      while (!argv.error && ((_a = argv.tokens) == null ? void 0 : _a.length)) {
        ...
        if (content[0] !== "-" && resolveConfig2(argDecl == null ? void 0 : argDecl.type).greedy) {
        ...
        if (!quoted && (option = this._symbolicOptions[content])) {
          names = [(0, import_cosmokit6.paramCase)(option.name)];
        } else {
          if (content[0] !== "-" || quoted) {
          ...
          if (content.slice(i, i + 3) === "no-" && !this._namedOptions[content.slice(i)]) {
            ...
            options[(0, import_cosmokit6.camelCase)(name)] = false;
            continue;
          }
          let j = i + 1;
        ...
        if (!param) {
          ...
          if (resolveConfig2(type).greedy) {
            ...
            quoted = true;
            argv.tokens = [];
          } else if (type !== "boolean" && argv.tokens.length && (type || ((_b = argv.tokens[0]) == null ? void 0 : _b.content) !== "-")) {
        ...
        for (let j = 0; j < names.length; j++) {
          ...
          const key = optDecl ? optDecl.name : (0, import_cosmokit6.camelCase)(name);
...
// koishi/packages/core/src/command/command.ts
var logger2 = new import_core9.Logger("command");
var _Command = class extends Argv.CommandBase {
  ...
  _registerAlias(name, prepend = false) {
    ...
    if (done) {
      if (prepend) {
        (0, import_cosmokit7.remove)(this._aliases, name);
  ...
  shortcut(name, config = {}) {
    ...
    (_a = this._disposables) == null ? void 0 : _a.push(() => (0, import_cosmokit7.remove)(this.ctx.$commander._shortcuts, config));
    return this;
  }
  subcommand(def, ...args) {
  ...
  before(callback, append = false) {
    ...
    (_a = this._disposables) == null ? void 0 : _a.push(() => (0, import_cosmokit7.remove)(this._checkers, callback));
    return this;
  }
  action(callback, prepend = false) {
    ...
    (_a = this._disposables) == null ? void 0 : _a.push(() => (0, import_cosmokit7.remove)(this._actions, callback));
    return this;
  }
  use(callback, ...args) {
  ...
  async execute(argv, fallback = Next.compose) {
    ...
    if (error)
      return error;
    if (logger2.level >= 3)
      logger2.debug(argv.source || (argv.source = this.stringify(args, options)));
    ...
    try {
      ...
      if (!(0, import_cosmokit7.isNullable)(result))
        return result;
    } catch (error2) {
      if (index === length)
        throw error2;
      const stack = (0, import_utils6.coerce)(error2);
      logger2.warn(`${argv.source || (argv.source = this.stringify(args, options))}
${stack}`);
  ...
  dispose() {
    ...
    (0, import_cosmokit7.remove)(this.ctx.$commander._commandList, this);
    if (this.parent) {
      (0, import_cosmokit7.remove)(this.parent.children, this);
    }
  }
};
...
((Command2) => {
  Command2.Config = import_core9.Schema.object({
    authority: import_core9.Schema.union([
      import_core9.Schema.natural(),
      import_core9.Schema.function()
    ]).default(1),
    hidden: import_core9.Schema.boolean().default(false),
    checkArgCount: import_core9.Schema.boolean().default(false),
    checkUnknown: import_core9.Schema.boolean().default(false)
  });
})(Command || (Command = {}));

// koishi/packages/core/src/command/runtime.ts
var import_utils7 = require("@koishijs/utils");
function runtime(ctx) {
  ...
  ctx.before("parse", (content, session) => {
    ...
    for (const shortcut of ctx.$commander._shortcuts) {
      ...
      if (prefix && !parsed.appel || !command.ctx.filter(session))
        continue;
      if (quote)
      ...
      if (typeof name === "string") {
        ...
        let escape = function(source) {
          ...
          capture.forEach((segment4, index) => {
            if (!index || index > 9)
              return;
        ...
        return command.parse({
          ...
          options: (0, import_utils7.valueMap)(options, escape)
        });
      }
    }
  });
  ctx.before("attach", (session) => {
    (0, import_utils7.defineProperty)(session, "argv", ctx.bail("before-parse", session.parsed.content, session));
...
var Commander = class {
  constructor(ctx, config = {}) {
    ...
    this._shortcuts = [];
    (0, import_cosmokit8.defineProperty)(this, import_core10.Context.current, ctx);
    ctx.plugin(runtime);
    ctx.plugin(validate);
  }
  get caller() {
    return this[import_core10.Context.current];
  }
  resolve(key) {
    if (!key)
      return;
...
Commander.methods = ["command"];
import_core10.Context.service(Commander.key, Commander);

// koishi/packages/core/src/session.ts
var logger3 = new import_core11.Logger("session");
var { initialize } = import_core11.Session.prototype;
(0, import_utils8.extend)(import_core11.Session.prototype, {
  initialize() {
    initialize.call(this);
    (0, import_cosmokit9.defineProperty)(this, "scope", null);
    (0, import_cosmokit9.defineProperty)(this, "user", null);
    (0, import_cosmokit9.defineProperty)(this, "channel", null);
    (0, import_cosmokit9.defineProperty)(this, "guild", null);
    (0, import_cosmokit9.defineProperty)(this, "_queuedTasks", []);
    (0, import_cosmokit9.defineProperty)(this, "_queuedTimeout", null);
  },
  get username() {
  ...
  async send(content, options = {}) {
    ...
    return this.bot.sendMessage(this.channelId, content, this.guildId, options).catch((error) => {
      logger3.warn(error);
      return [];
    });
  },
  ...
  async sendQueued(content, delay) {
    const text = import_core11.segment.normalize(content).join("");
    if (!text)
      return;
  ...
  async _observeChannelLike(channelId, fields = []) {
    ...
    if (cache) {
      ...
      cache = (0, import_utils8.observe)(data, (diff) => this.app.database.setChannel(platform, channelId, diff), `channel ${key}`);
  ...
  async observeUser(fields = []) {
    ...
    if ((_a = this.author) == null ? void 0 : _a.anonymous) {
      ...
      const user = (0, import_utils8.observe)(fallback, () => Promise.resolve());
    ...
    if (cache) {
      ...
      cache = (0, import_utils8.observe)(data, (diff) => this.app.database.setUser(this.platform, userId, diff), `user ${this.uid}`);
  ...
  text(path, params = {}) {
    ...
    const paths = (0, import_cosmokit9.makeArray)(path).map((path2) => {
      if (!path2.startsWith("."))
        return path2;
      if (!this.scope) {
  ...
  async execute(argv, next) {
    ...
    if (argv.tokens) {
      ...
      argv.command || (argv.command = this.app.$commander.getCommand(argv.name));
      if (!argv.command) {
    ...
    return this.withScope(`commands.${command.name}.messages`, async () => {
      ...
      if (!shouldEmit) {
        if (typeof result === "string")
          return result;
        return (0, import_core11.segment)(null, result).toString();
      }
