...
import { Context as Context3 } from "@satorijs/core";
...
var FilterService = class {
  constructor(app) {
    this.app = app;
    defineProperty3(this, Context3.current, app);
    app.filter = () => true;
...
__publicField(FilterService, "methods", [
  "any",
  "never",
  "union",
  "intersect",
...
Context3.service("$filter", FilterService);

// koishi/packages/core/src/i18n.ts
import { Random } from "@koishijs/utils";
import { distance } from "fastest-levenshtein";
import { isNullable } from "cosmokit";
import { Context as Context4, Logger, segment } from "@satorijs/core";
...
var I18n = class {
  ...
  set(locale, prefix, value) {
    if (prefix.includes("@") || typeof value === "string") {
      ...
      dict[path] = value;
      this[Context4.current]?.on("dispose", () => {
        delete dict[path];
      });
    } else {
...
__name(I18n, "I18n");
Context4.service("i18n", I18n);

// koishi/packages/core/src/middleware.ts
import { coerce, escapeRegExp, makeArray, Random as Random2 } from "@koishijs/utils";
import { defineProperty as defineProperty4, Time as Time2 } from "cosmokit";
import { Context as Context5, segment as segment2 } from "@satorijs/core";
...
var Processor = class {
  constructor(ctx, config) {
    this.ctx = ctx;
    this.config = config;
    defineProperty4(this, Context5.current, ctx);
    this.prepare();
  ...
  _executeMatcher(session, matcher) {
    ...
    session.response = async () => {
      ...
      return segment2.normalize(output, params.map((source) => source ? segment2.parse(source) : ""));
    };
  }
  prepare() {
  ...
  async _process(session, next) {
    ...
    defineProperty4(session, "parsed", { content, appel, prefix });
...
__publicField(Processor, "methods", ["middleware", "match"]);
Context5.service("$internal", Processor);
...
import { Context as Context6 } from "@satorijs/core";
import { extend as extend2 } from "@koishijs/utils";
extend2(Context6.prototype, {
  get app() {
    return this.root;
  },
  get options() {
...
import { defineProperty as defineProperty6, isNullable as isNullable3, makeArray as makeArray2 } from "cosmokit";
import { Context as Context10, Logger as Logger4, segment as segment4, Session as Session6 } from "@satorijs/core";
...
import { Context as Context9 } from "@satorijs/core";
...
var Commander = class {
  constructor(ctx, config = {}) {
    this.ctx = ctx;
    this.config = config;
    defineProperty5(this, Context9.current, ctx);
    ctx.plugin(validate);
    ...
    ctx.before("attach", (session) => {
      defineProperty5(session, "argv", ctx.bail("before-parse", session.parsed.content, session));
...
Context9.service(Commander.key, Commander);

// koishi/packages/core/src/session.ts
var logger3 = new Logger4("session");
var { initialize } = Session6.prototype;
extend3(Session6.prototype, {
  [Context10.filter](ctx) {
  ...
  text(path, params = {}) {
    ...
    const paths = makeArray2(path).map((path2) => {
      if (!path2.startsWith("."))
        return path2;
      if (!this.scope) {
