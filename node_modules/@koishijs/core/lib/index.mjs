...
DatabaseService.prototype.extend = /* @__PURE__ */ __name(function extend(name, fields, config) {
  Database.prototype.extend.call(this, name, fields, {
    ...config,
    driver: this[Context2.current].mapping.database
  });
...
import { isNullable, Time as Time2 } from "cosmokit";
import { Context as Context3, Logger } from "@satorijs/core";
...
var I18n = class {
  ...
  registerBuiltins() {
    ...
    this.formatter("time", (ms, _, locale) => {
      ...
      return Math.round(ms / Time2.second) + " " + this.text([locale], ["general.second"], {});
    });
...
import { coerce, escapeRegExp, makeArray as makeArray2 } from "@koishijs/utils";
import { defineProperty as defineProperty3 } from "cosmokit";
import { Context as Context4, segment } from "@satorijs/core";
...
__publicField(Internal, "methods", ["middleware"]);
...
import { defineProperty as defineProperty6, isNullable as isNullable3, makeArray as makeArray3 } from "cosmokit";
import { Logger as Logger4, segment as segment3, Session as Session4 } from "@satorijs/core";
...
import { isNullable as isNullable2, remove as remove2 } from "cosmokit";
...
import { segment as segment2 } from "@satorijs/core";
var leftQuotes = `"'“‘`;
var rightQuotes = `"'”’`;
var Argv;
((Argv2) => {
  ...
  class Tokenizer {
    ...
    parse(source, terminator = "") {
      const tokens = [];
      source = segment2.parse(source).map((el) => {
  ...
  createDomain("rawtext", (source) => segment2.unescape(source), { greedy: true });
  ...
  createDomain("user", (source, session) => {
    ...
    const code = segment2.from(source);
...
var _Command = class extends Argv.CommandBase {
  ...
  shortcut(name, config = {}) {
    ...
    this._disposables?.push(() => remove2(this.ctx.$commander._shortcuts, config));
    return this;
  }
  subcommand(def, ...args) {
...
import { defineProperty as defineProperty4, valueMap } from "@koishijs/utils";
...
var Commander = class {
  constructor(ctx, config = {}) {
    this.ctx = ctx;
    this.config = config;
    defineProperty5(this, Context8.current, ctx);
    ctx.plugin(runtime);
    ctx.plugin(validate);
  }
  _commandList = [];
...
var { initialize } = Session4.prototype;
extend3(Session4.prototype, {
  initialize() {
    initialize.call(this);
    defineProperty6(this, "scope", null);
    defineProperty6(this, "user", null);
    defineProperty6(this, "channel", null);
    defineProperty6(this, "guild", null);
    defineProperty6(this, "_queuedTasks", []);
    defineProperty6(this, "_queuedTimeout", null);
  },
  get username() {
  ...
  async sendQueued(content, delay) {
    const text = segment3.normalize(content).join("");
    if (!text)
      return;
  ...
  async execute(argv, next) {
    ...
    return this.withScope(`commands.${command.name}.messages`, async () => {
      ...
      if (!shouldEmit) {
        if (typeof result === "string")
          return result;
        return segment3(null, result).toString();
      }
