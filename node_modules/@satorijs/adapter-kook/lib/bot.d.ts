...
export declare class KookBot<T extends KookBot.Config = KookBot.Config> extends Bot<T> {
    ...
    request<T = any>(method: Method, path: string, data?: any, headers?: any): Promise<T>;
    sendMessage(channelId: string, content: string | segment, guildId?: string): any;
    sendPrivateMessage(target_id: string, content: string | segment): any;
    deleteMessage(channelId: string, msg_id: string): Promise<void>;
    editMessage(channelId: string, msg_id: string, content: string | segment): Promise<void>;
    ...
    getGuildMemberList(): Promise<import("@satorijs/core").Universal.Author[]>;
...
export declare namespace KookBot {
    interface BaseConfig extends Bot.Config, Quester.Config, KookModulator.Config {
    }
