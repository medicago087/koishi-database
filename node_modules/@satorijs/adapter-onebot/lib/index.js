...
__export(src_exports, {
  ...
  OneBotBot: () => OneBotBot,
  OneBotMessenger: () => OneBotMessenger,
...
async function adaptMessage(bot, message, result = {}) {
  ...
  result.elements = import_satori2.segment.transform(chain, {
    at({ qq }) {
      if (qq !== "all")
    ...
    face({ id }) {
      return (0, import_satori2.segment)("face", { id, platform: bot.platform }, [
        import_satori2.segment.image(qface.getUrl(id))
      ]);
    },
    record(attrs) {
      return (0, import_satori2.segment)("audio", attrs);
    }
  });
...
// satori/adapters/onebot/src/bot/message.ts
...
__name(State, "State");
var OneBotMessenger = class extends import_satori5.Messenger {
  constructor() {
    super(...arguments);
  ...
  async visit(element) {
    ...
    if (type === "text") {
      ...
      if ("forward" in attrs && !this.bot.parent) {
        ...
        Object.assign(this.stack[0].author, (0, import_satori5.pick)(attrs, ["userId", "username", "nickname", "avatar"]));
        await this.render(children);
        await this.flush();
      }
    } else {
...
__name(OneBotMessenger, "OneBotMessenger");

// satori/adapters/onebot/src/bot/base.ts
var BaseBot2 = class extends import_satori6.Bot {
  sendMessage(channelId, fragment, guildId, options) {
    if (!this.parent && !channelId.startsWith("private:")) {
      guildId = channelId;
    }
    return new OneBotMessenger(this, channelId, guildId, options).send(fragment);
  }
  sendPrivateMessage(userId, fragment, options) {
...
__name(BaseBot2, "BaseBot");
((BaseBot3) => {
  BaseBot3.AdvancedConfig = import_satori6.Schema.object({
    splitMixedContent: import_satori6.Schema.boolean().description("是否自动在混合内容间插入空格。").default(true)
  }).description("高级设置");
})(BaseBot2 || (BaseBot2 = {}));
