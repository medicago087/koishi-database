...
((Element2) => {
  ...
  function createAssetFactory(type) {
    return (url, ...args) => {
      ...
      if ((0, import_cosmokit.is)("Buffer", url)) {
        url = prefix + url.toString("base64");
      } else if ((0, import_cosmokit.is)("ArrayBuffer", url)) {
        url = prefix + (0, import_cosmokit.arrayBufferToBase64)(url);
      }
      if (url.startsWith("base64://")) {
        Element2.warn(`protocol "base64:" is deprecated and will be removed in the future, please use "data:" instead`);
      }
      return Element2(type, { ...args[0], url });
    };
  }
